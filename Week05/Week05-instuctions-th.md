...
3. มี `Random rng = new Random(14);` ตัวเดียว เรียก `rng.Next()` ใช้จากในไฟล์นี้ หรือ ใน Class ที่สอนทำก็ได้
